...
function category_options(?int $selected = null): string {
    ...
    foreach (categories() as $cat) {
        $isSelected = ((int) $cat['id'] === (int) $selected) ? ' selected' : '';
        $html .= '<option value="' . h((string) $cat['id']) . '"' . $isSelected . '>' . h($cat['name']) . '</option>';
    }
    return $html;
}
...
function handle_post(): void {
    ...
    if (isset($_POST['save_link'])) {
        ...
        $fakeSlug = trim((string) ($_POST['fake_slug'] ?? ''));
        if ($fakeSlug === '') {
            $fakeSlug = preg_replace('/[^a-z0-9-]+/i', '-', strtolower($id));
        }
        $stmt = db()->prepare('INSERT INTO links (id, title, description, image, redirect_url, category_id, fake_slug, preview_mode) VALUES (:id, :title, :description, :image, :redirect_url, :category_id, :fake_slug, :preview_mode)
            ON CONFLICT(id) DO UPDATE SET title = excluded.title, description = excluded.description, image = excluded.image, redirect_url = excluded.redirect_url, category_id = excluded.category_id, fake_slug = excluded.fake_slug, preview_mode = excluded.preview_mode');
        $stmt->execute([
            ':id' => $id,
...
function render_public(): void {
    ...
    if ($linkId !== '' || $imageSlug !== '') {
        if ($imageSlug !== '') {
            $stmt = db()->prepare('SELECT * FROM links WHERE fake_slug = :id');
            $stmt->execute([':id' => $imageSlug]);
        } else {
    ...
    <main class="admin-main">
      <section class="panel">
        ...
        <form method="post" class="form-grid">
          ...
          <label>Ảnh OG / ảnh hiển thị<input name="image" value="<?= h($edit['image'] ?? '') ?>" placeholder="https://..."></label>
          <label>Redirect URL<input name="redirect_url" value="<?= h($edit['redirect_url'] ?? '') ?>" placeholder="https://..." required></label>
          <label>Phân loại<select name="category_id"><?= category_options(isset($edit['category_id']) ? (int) $edit['category_id'] : null) ?></select></label>
          <label>Slug link ảnh ẩn<input name="fake_slug" value="<?= h($edit['fake_slug'] ?? '') ?>" placeholder="anh-hd-rieng-tu"></label>
          <label class="check-row"><input type="checkbox" name="preview_mode" value="1" <?= !empty($edit['preview_mode']) ? 'checked' : '' ?>> Bật trang xem ảnh trước khi chuyển hướng</label>
      ...
      <section class="panel">
        ...
        <form method="post" class="form-grid">
          ...
          <label>Màu chính<input type="color" name="brand_color" value="<?= h(setting('brand_color')) ?>"></label>
          <label>Màu phụ<input type="color" name="accent_color" value="<?= h(setting('accent_color')) ?>"></label>
          <label>Bo góc card (px)<input type="number" min="8" max="48" name="card_radius" value="<?= h(setting('card_radius')) ?>"></label>
          <label>CSS tùy chỉnh<textarea name="custom_css" rows="4" placeholder=".verify-card{...}"><?= h(setting('custom_css')) ?></textarea></label>
      ...
      <section class="panel">
        ...
        <form method="post" class="form-grid">
          ...
          <label>Tên phân loại<input name="category_name" placeholder="Ví dụ: Ảnh, Tin tức, Khách hàng" required></label>
          <label>Màu<input type="color" name="category_color" value="#2563eb"></label>
          <button class="button" type="submit">Thêm / cập nhật phân loại</button>
        </form>
        <div class="chips">
          <?php foreach (categories() as $cat): ?>
            <form method="post" class="chip-form" onsubmit="return confirm('Xóa phân loại này?')">
              <input type="hidden" name="delete_category" value="1">
              <input type="hidden" name="category_id" value="<?= h((string) $cat['id']) ?>">
              <button class="chip" style="--chip: <?= h($cat['color']) ?>"><?= h($cat['name']) ?> ×</button>
            </form>
          <?php endforeach; ?>
        </div>
      </section>
      ...
      <section class="panel full">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Meta</th><th>URL</th><th>Link ảnh</th><th>Lượt mở</th><th>Hành động</th></tr></thead>
            <tbody>
            <?php foreach ($links as $link): ?>
              <?php $publicUrl = current_url(['v' => $link['id']]); ?>
              <tr>
                <td><strong><?= h($link['title']) ?></strong><br><code><?= h($link['id']) ?></code><?php if (!empty($link['category_name'])): ?><br><span class="mini-chip" style="--chip: <?= h($link['category_color']) ?>"><?= h($link['category_name']) ?></span><?php endif; ?></td>
                <td><a href="<?= h($publicUrl) ?>"><?= h($publicUrl) ?></a><br><small><?= h($link['redirect_url']) ?></small></td>
                <td><?php if (!empty($link['fake_slug'])): ?><a href="<?= h(current_url(['img' => $link['fake_slug']])) ?>"><?= h(current_url(['img' => $link['fake_slug']])) ?></a><?php else: ?>—<?php endif; ?></td>
            ...
            <?php endforeach; ?>
            <?php if (!$links): ?><tr><td colspan="5">Chưa có link nào.</td></tr><?php endif; ?>
            </tbody>
          </table>
        </div>
      </section>
